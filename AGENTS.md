...
**能拆就并行：** 任务只要能切成互不重叠的子活儿（多文件只读排查、写入范围不冲突的多处改动、独立验证），就 spawn 多个子 agent 同时做，别一条道串行。子任务要自包含、写文件范围两两不相交，回来自己整合；但紧挨着下一步的阻塞活儿留给自己做，spawn 完别干等。
